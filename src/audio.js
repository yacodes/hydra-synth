...
class Audio {
  constructor({
    ...
    window.navigator.mediaDevices.enumerateDevices().then(darr => {
    ...
    if (this.meyda) {
      ...
      if (features && features !== null) {
        ...
        this.fft = this.bins.map((bin, index) =>
          ...
          Math.max(
            0,
            (bin - this.settings[index].cutoff) / this.settings[index].scale,
          ),
        );
  ...
  draw() {
    ...
    this.bins.forEach((bin, index) => {
      ...
      var yMax =
        this.canvas.height -
        scale * (this.settings[index].scale + this.settings[index].cutoff);
      this.ctx.beginPath();
